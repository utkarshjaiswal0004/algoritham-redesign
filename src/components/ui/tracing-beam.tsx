...
export function TracingBeam({ children, className = "" }: { children: ReactNode; className?: string }) {
  ...
  return (
    <motion.div ref={ref} className={`relative w-full mx-auto ${className}`}>
      <div className="absolute -left-4 md:-left-12 top-3">
        <motion.div
          ...
          className="ml-[27px] h-4 w-4 rounded-full border border-[var(--border)] shadow-sm flex items-center justify-center bg-[var(--bg-card)]"
        >
          <motion.div
            transition={{ duration: 0.2, delay: 0.5 }}
            className="h-2 w-2 rounded-full border border-[var(--brand-purple)] bg-[var(--brand-purple)]"
          />
        </motion.div>
        <svg
